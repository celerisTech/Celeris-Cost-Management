...
export async function GET(request: NextRequest) {
    try {
        ...
        const dateFallback = new Date().toISOString().split("T")[0];
        const startDate = searchParams.get("startDate") || dateFallback;
        const endDate = searchParams.get("endDate") || dateFallback;
        const type = searchParams.get("type") || "tasks"; // 'tasks' or 'history'
        const db = await getDb();

        if (type === "history") {
            // Query to fetch all task updates for the selected date range across all projects
            const historyQuery = `
                SELECT 
                  tu.CM_Update_ID,
                  tu.CM_Task_ID,
                ...
                WHERE tu.CM_Update_Date >= ? AND tu.CM_Update_Date < DATE_ADD(?, INTERVAL 1 DAY)
                ORDER BY tu.CM_Update_Date DESC
            `;
            const [historyRows] = await db.query<RowDataPacket[]>(historyQuery, [startDate, endDate]);
        ...
        LEFT JOIN (
            ...
            WHERE tu1.CM_Update_ID IN (
                SELECT MAX(CM_Update_ID)
                FROM ccms_task_update
                WHERE CM_Update_Date < DATE_ADD(?, INTERVAL 1 DAY)
                GROUP BY CM_Task_ID
            )
        ) tu 
            ON t.CM_Task_ID = tu.CM_Task_ID
        LEFT JOIN ccms_users u 
            ON t.CM_Engineer_ID = u.CM_User_ID
        WHERE t.CM_Assign_Date < DATE_ADD(?, INTERVAL 1 DAY)
        AND (
            t.CM_Due_Date IS NULL
            OR t.CM_Due_Date >= ?
        )
        AND (
            tu.CM_Status IS NULL 
            OR tu.CM_Status != 'Completed'
            OR tu.CM_Update_Date >= ?
        )
        ORDER BY p.CM_Project_Name ASC, t.CM_Task_Name ASC
        `;


        const [rows] = await db.query<RowDataPacket[]>(query, [endDate, endDate, startDate, startDate]);
