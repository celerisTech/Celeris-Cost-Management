    ...
    if (type === 'engineers') {
      ...
      const [projects] = await db.query(`
        SELECT 
          ...
          c.CM_GST_Number,
          c.CM_PAN_Number,
          (SELECT COUNT(DISTINCT CM_Task_ID) FROM ccms_task_master WHERE CM_Project_ID = p.CM_Project_ID) AS totalTasks,
          (SELECT COUNT(DISTINCT t.CM_Task_ID) 
           ...
           JOIN (
              ...
              INNER JOIN (
                ...
                GROUP BY CM_Task_ID
              ) latest ON tu.CM_Task_ID = latest.CM_Task_ID AND tu.CM_Update_Date = latest.latest_update
           ) u ON t.CM_Task_ID = u.CM_Task_ID
           WHERE t.CM_Project_ID = p.CM_Project_ID AND u.CM_Status = 'Completed'
          ) AS completedTasks
      FROM ccms_projects p
