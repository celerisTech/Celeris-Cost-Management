    ...
    <div className="p-4 max-w-5xl mx-auto space-y-4 text-gray-800">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-3 border-b border-gray-200">
        <div>
          ...
          <p className="text-xs text-gray-500">Manage the dropdown options for Visit Statuses</p>
        </div>
          ...
          <table className="w-full text-left border-collapse table-fixed">
            <thead>
              <tr className="bg-[#eef2ff] text-blue-700 border-b border-gray-200">
                ...
                <th className="px-4 py-2.5 text-xs font-bold uppercase tracking-wider border border-slate-200 text-blue-700">Status Name</th>
            ...
            <tbody>
              {loading ? (
                ...
                statuses.map((status, idx) => (
                  <tr key={status.Status_ID} className={`hover:bg-blue-50/20 transition-colors border border-slate-200 ${idx % 2 === 0 ? 'bg-[#f4f7ff]/50' : 'bg-white'}`}>
                    <td className="px-4 py-2 text-xs font-semibold text-gray-500 text-center border border-slate-200">#{status.Status_ID}</td>
                    <td className="px-4 py-2 text-sm font-bold text-gray-800 border border-slate-200 truncate">{status.Status_Name}</td>
                    ...
                    <td className="px-4 py-2 border border-slate-200 text-center">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-[11px] font-bold border ${status.Is_Active ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-gray-100 text-gray-600 border-gray-200"}`}>
                        {status.Is_Active ? <CheckCircle2 size={12} /> : <AlertCircle size={12} />}
          ...
          <div className="bg-white rounded-sm shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-700 uppercase">Status Name *</label>
                <input
                  type="text"
                  required
                ...
                <div className="space-y-1">
                  <label className="flex items-center gap-3 cursor-pointer p-2.5 border border-amber-200 bg-amber-50/30 rounded-sm hover:bg-amber-50/50 transition-colors">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-amber-600">Update past visits that used this status name</div>
                    </div>
                  </label>
                </div>
              )}
