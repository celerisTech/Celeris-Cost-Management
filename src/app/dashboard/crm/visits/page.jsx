    ...
    <div className="p-2 md:p-2 h-[calc(100vh-16px)] flex flex-col justify-between space-y-3 overflow-hidden text-gray-800 w-full max-w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          >
            <Plus className="h-4 w-4" /> Add Visits
          </button>
        </div>
      </div>

      {/* Filters Card */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 items-end text-gray-800 w-full overflow-visible pb-3 pt-2 sticky top-0 z-20 border-b px-2">
      ...
      <div className="flex-1 min-h-0 flex flex-col !-mt-2 w-full max-w-full overflow-hidden">
        {viewMode === "table" ? (
          <div className="flex bg-white border border-gray-200 overflow-hidden shadow-sm rounded-sm flex-col flex-1 min-h-0 w-full max-w-full">
              ...
              <table className="w-full min-w-[1100px] text-left border-collapse table-fixed">
                <thead className="sticky top-0 z-10 shadow-sm">
                  <tr className="bg-[#eef2ff] text-blue-700">
                    ...
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-32 border border-slate-200 text-blue-700">Visit Date</th>
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-48 border border-slate-200 text-blue-700">Client / Company</th>
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-64 border border-slate-200 text-blue-700">Purpose & Description</th>
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-32 border border-slate-200 text-blue-700">Next Followup</th>
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-20 border border-slate-200 text-blue-700">Type</th>
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-24 border border-slate-200 text-blue-700">Status</th>
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-28 border border-slate-200 text-blue-700">Product</th>
                    <th className="px-2 py-2 text-[12px] font-bold uppercase w-24 text-center border border-slate-200 text-blue-700">Actions</th>
                  </tr>
                </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan="9" className="px-6 py-2 text-center text-gray-500 border border-slate-200">No visits found</td></tr>
                ) : (
                  Object.entries(
                    visits.reduce((acc, v) => {
                    ...
                    <React.Fragment key={execName}>
                      <tr className="bg-gray-100">
                        <td colSpan="9" className="px-2 py-1.5 font-bold text-gray-800 border border-slate-200">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-sm text-blue-600">{execName}</span> <span className="text-[12px] font-normal text-gray-600 bg-gray-200 px-1.5 py-0.5 rounded-sm border border-gray-300">{execVisits.length} Visits</span>
                          </div>
                        </td>
                      </tr>
                      {execVisits.map((v, idx) => (
                        <tr
                          key={v.CM_Visit_ID}
                          className={`hover:bg-blue-50/20 transition-colors cursor-pointer border border-slate-200 ${idx % 2 === 0 ? 'bg-[#f4f7ff]/50' : 'bg-white'}`}
                        >
                          <td className="px-2 py-1.5 text-[12px] text-gray-500 text-center border border-slate-200">{(page - 1) * limit + idx + 1}</td>
                          ...
                          <td className="px-2 py-1.5 border border-slate-200">
                            ...
                            <p className="text-[12px] text-gray-500 truncate">{v.CM_Company_Name || "Individual"}</p>
                          ...
                          <td className="px-2 py-1.5 border border-slate-200 text-center">
                            <div className="flex justify-center gap-1.5">
                              ...
                              <button
                                onClick={() => handleDelete(v.CM_Visit_ID)}
        ...
        <div className="space-y-4 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-0.5 before:bg-blue-100 pl-10 overflow-auto flex-1 min-h-0 pr-2">
          {visits.map((v) => (
