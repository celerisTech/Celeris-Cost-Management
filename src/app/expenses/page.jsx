...
export default function ProjectCostReport() {
  ...
  const [selectedProgress, setSelectedProgress] = useState("In Progress"); // New progress filter state
  ...
  useEffect(() => {
    ...
    if (selectedProgress !== "all") {
      filtered = filtered.filter(project => {
        ...
        const progress = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;
        const isCompleted = (totalTasks > 0 && progress === 100) || (project.CM_Status === 'Completed');
        return selectedProgress === 'Completed' ? isCompleted : !isCompleted;
      });
    }
    ...
    setSelectAll(false);
  }, [selectedProjectType, selectedProgress, searchTerm, projects]);
  ...
  const exportToExcel = async (selectedOnly = false) => {
    ...
    const wsData = [
      ...
      [],
      ["Project Type", "Project Code", "Name", "Estimated Cost", "Actual Cost", "Variance"],
      ...dataToExport.map(p => [
  ...
  return (
    <div className="flex flex-row h-screen bg-white">
      ...
      {/* Main content */}
      <div className="flex-1 overflow-y-auto p-2 sm:p-3 lg:p-3 w-full">
        <div className="bg-white overflow-hidden max-w-[1800px] mx-auto">
          ...
          <div className="p-4 sm:p-3 border-b border-gray-200">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
              ...
              <div className="flex-shrink-0">
                <h2 className="text-xl sm:text-2xl lg:text-2xl font-bold text-gray-900 flex items-center gap-2 lg:gap-3">
              ...
              <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                  ...
                  <div className="flex bg-gray-100 p-1 rounded-lg w-full sm:w-auto">
                    <button
                      ...
                      className={`flex-1 sm:flex-none px-4 py-1.5 text-xs sm:text-sm font-medium rounded-md transition-all duration-200 ${selectedProgress === 'In Progress' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                      In Progress ({progressCounts['In Progress']})
                    </button>
                    <button
                      onClick={() => setSelectedProgress('Completed')}
                      className={`flex-1 sm:flex-none px-4 py-1.5 text-xs sm:text-sm font-medium rounded-md transition-all duration-200 ${selectedProgress === 'Completed' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                      Completed ({progressCounts['Completed']})
                    </button>
                    <button
                      onClick={() => setSelectedProgress('all')}
                      className={`flex-1 sm:flex-none px-4 py-1.5 text-xs sm:text-sm font-medium rounded-md transition-all duration-200 ${selectedProgress === 'all' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                ...
                <div className="relative inline-block text-left">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 sm:px-4 sm:py-2 bg-gray-700 text-white rounded-lg shadow-md hover:bg-gray-800 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 text-sm sm:text-base"
                  >
            ...
            {(selectedProjectType !== 'all' || selectedProgress !== 'all' || searchTerm) && (
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <div className="flex items-center text-xs sm:text-sm text-gray-500 mr-2">
                ...
                {selectedProgress !== 'all' && (
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${selectedProgress === 'Completed' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-blue-100 text-blue-800 border-blue-200'}`}>
                    Progress: {selectedProgress}
                    <button
                      onClick={() => setSelectedProgress('all')}
                      className={`ml-1.5 focus:outline-none ${selectedProgress === 'Completed' ? 'text-green-600 hover:text-green-800' : 'text-blue-600 hover:text-blue-800'}`}
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ...
                {searchTerm && (
                  <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-200">
                    ...
                    <button
                      ...
                      className="ml-1.5 text-yellow-600 hover:text-yellow-800 focus:outline-none"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ...
                <button
                  ...
                  className="text-xs sm:text-sm text-gray-600 hover:text-gray-900 font-medium ml-2"
                >
                  Clear all
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {[
                ...
                <div
                  key={index}
                  className={`bg-white p-3 sm:p-2 rounded-xl border-l-4 shadow-sm hover:shadow-md transition-all duration-200 ${card.borderColor ? card.borderColor : 'border-gray-200'}`}
                >
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs sm:text-sm">
                      <div>
                        ...
                        <p className="font-bold text-gray-900 truncate">{formatIndianRupees(project.CM_Estimated_Cost)}</p>
                      </div>
                      <div>
                        <p className="text-gray-500 text-xs mb-1">Actual</p>
                        <p className="font-bold text-gray-900 truncate">{formatIndianRupees(project.Actual_Cost)}</p>
                      </div>
                      <div>
                        <p className="text-gray-500 text-xs mb-1">Variance</p>
                        <p className={`font-bold truncate ${project.Cost_Variance < 0 ? 'text-red-600' : 'text-green-600'}`}>
          ...
          {/* Desktop View: Full Excel-style Table Layout (1024px and above) */}
          <div className="hidden lg:block overflow-x-auto border-t border-gray-300">
            <table className="w-full border-collapse text-xs">
              <thead className="bg-[#f3f4f6] sticky top-0 z-20 shadow-sm border-b-2 border-gray-300">
                <tr className="text-left font-bold text-gray-700">
                  <th className="px-2 py-2 border-r border-l border-gray-300 w-10 text-center bg-gray-100">
                    <button
                      onClick={toggleSelectAll}
                      className={`w-4 h-4 rounded border flex items-center justify-center transition-all mx-auto ${selectAll
                        ? 'bg-blue-600 border-blue-600 text-white'
                        : 'bg-white border-gray-400 hover:border-blue-500'
                        }`}
                    >
                      {selectAll && <Check className="w-2.5 h-2.5" />}
                    </button>
                  </th>
                  <th className="px-2 py-2 border-r border-l border-gray-300 w-10 text-center bg-gray-100">#</th>
                  {[
                    { key: 'project_info', label: 'PROJECT NAME & CODE', sortable: false, width: 'min-w-[300px]' },
                    { key: 'CM_Project_Type', label: 'PROJECT TYPE', sortable: true, width: 'w-[12%]' },
                    { key: 'CM_Estimated_Cost', label: 'ESTIMATED BUDGET', sortable: true, align: 'right', width: 'w-[12%]' },
                    { key: 'Actual_Cost', label: 'ACTUAL EXPENSE', sortable: true, align: 'right', width: 'w-[12%]' },
                    { key: 'Cost_Variance', label: 'COST VARIANCE', sortable: true, align: 'right', width: 'w-[12%]' }
                  ].map(({ key, label, sortable, align = 'left', width }) => (
                    <th
                      key={key}
                      className={`px-3 py-2 border-r border-l border-gray-300 text-${align} uppercase tracking-tighter cursor-pointer hover:bg-gray-200 transition-colors ${width}`}
                  ...
                  <th className="px-2 py-2 text-center bg-gray-100 border-r border-gray-300">ACTIONS</th>
                </tr>
              </thead>
              <tbody className="bg-white">
                  ...
                  >
                    <td className="px-2 py-1.5 border-r border-gray-200 text-center">
                      <button
                        ...
                        className={`w-4 h-4 rounded border flex items-center justify-center transition-all mx-auto ${selectedProjects.has(project.CM_Project_ID)
                          ? 'bg-blue-600 border-blue-600 text-white'
                    ...
                    <td className="px-2 py-1.5 border-r border-gray-200 text-center text-gray-500">{index + 1}</td>
                    <td className="px-3 py-1.5 border-r border-gray-200">
                      <div className="flex flex-col">
                        <span className="font-bold text-gray-800 uppercase leading-tight">{project.CM_Project_Name}</span>
                        <span className="text-[10px] text-blue-600 mt-0.5">{project.CM_Project_Code}</span>
                      </div>
                    </td>
                    <td className="px-3 py-1.5 border-r border-gray-200">
                    ...
                    <td className="px-3 py-1.5 border-r border-gray-200 text-sm text-right font-semibold text-gray-700 bg-gray-50/30">
                      {formatIndianRupees(project.CM_Estimated_Cost)}
                    </td>
                    <td className="px-3 py-1.5 border-r border-gray-200 text-sm text-right font-bold text-gray-900">
                      {formatIndianRupees(project.Actual_Cost)}
                    </td>
                    <td className="px-3 py-1.5 border-r border-gray-200 text-sm text-right">
                      <span className={`font-bold ${project.Cost_Variance < 0 ? 'text-red-600' : 'text-green-600'}`}>
                        {project.Cost_Variance < 0 ? '-' : '+'}
                        {formatIndianRupees(Math.abs(project.Cost_Variance))}
                      </span>
                    </td>
                    <td className="px-2 py-1.5 text-center border-r border-gray-200">
                      <button
                        ...
                        className="px-2 py-1 text-[10px] font-bold bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors uppercase"
                      >
                        Details
                      </button>
                    </td>
                ...
                <tr className="bg-gray-100 border-t-2 border-gray-300 font-bold">
                  <td colSpan={4} className="px-3 py-2 text-right text-gray-600">FILTERED TOTAL:</td>
                  <td className="px-3 py-2 text-right border-r border-gray-300 text-sm text-blue-700">{formatIndianRupees(totalEstimatedCost)}</td>
                  <td className="px-3 py-2 text-right border-r border-gray-300 text-sm text-indigo-700">{formatIndianRupees(totalActualCost)}</td>
                  <td colSpan={2} className="px-3 py-2 text-left text-sm">
                    <span className={totalVariance < 0 ? 'text-red-700' : 'text-green-700'}>
                      {totalVariance < 0 ? '-' : '+'}
                      {formatIndianRupees(Math.abs(totalVariance))}
                    </span>
                  </td>
                </tr>
              </tbody>
